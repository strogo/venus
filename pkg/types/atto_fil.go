...
func FilString(bi specsbig.Int) string {
	number1 := new(big.Float).SetInt(bi.Int)
	number2 := new(big.Float).SetInt(tenToTheEighteen.Int)
	return new(big.Float).Quo(number1, number2).String()
}
